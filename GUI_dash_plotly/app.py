...
for year, locations in mooring_data.items():
    year_options.append(html.Div([
        html.Label(year, style={'margin-right': '10px'}),
        dcc.Checklist(
            options=[{'label': location, 'value': f"{year}-{location}"} for location in locations],
...
app.layout = html.Div([
    ...
    dcc.Store(id='json-data-store'), # store the JSON data
])
...
def render_content(tab):
    if tab == 'tab-1':
        ...
        return html.Div([
            ...
            html.Button('Load JSON Data', id='load-json-data-button', n_clicks=0),
            html.Button('Render Plot', id='render-plot-button', n_clicks=0)
        ])
